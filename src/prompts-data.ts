...
语言硬性要求：所有标题、摘要、分析、趋势判断和建议必须使用自然、专业的简体中文，不得直接复制输入材料中的完整英文句子。Claude Code、Codex、GitHub、CLI、SDK、API、RAG、LLM 等产品名和技术名词可保留英文，但其作用与结论必须用中文解释。输出前逐段检查，除专有名词、代码、链接和版本号外不得出现英文段落。整体简洁专业，适合技术开发者快速掌握一周动态。
`;
}
...
语言硬性要求：所有标题、摘要、分析、趋势判断和建议必须使用自然、专业的简体中文，不得直接复制输入材料中的完整英文句子。Claude Code、Codex、GitHub、CLI、SDK、API、RAG、LLM 等产品名和技术名词可保留英文，但其作用、比较与结论必须用中文解释。输出前逐段检查，除专有名词、代码、链接和版本号外不得出现英文段落。内容需深度分析、数据驱动，适合月度复盘和战略决策参考。
`;
}
